...
grid = [[0 for i in range(row)] for j in range(column)]
...
def check_full_grid(grid:list):
    if check_rowcolumn(len(grid),len(grid[0]),grid):
        if check_square(len(grid),len(grid[0]),grid):
            return True
    return False


#the check_rowcolumn function is to check if input number is duplciated in row/column
def check_rowcolumn(current_row:int,current_column:int,grid:list,number:int): #1,8
    for i in range(len(grid)):
...
def check_square(current_row:int,current_column:int,grid:list,number:int):
...
def check_solution(cur_grid:list):
    global count
    #define base case
